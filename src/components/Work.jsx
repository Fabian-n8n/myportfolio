  ...
  return (
    ...
    >
      ...
      <motion.a
        ...
        className={`py-8 md:py-12 block ${project.href ? 'cursor-pointer' : 'cursor-default'}`}
        whileHover={project.href ? { x: 4 } : {}}
      ...
      >
        ...
        <div className="md:hidden">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-[11px] tracking-[0.18em] uppercase text-ink/35 font-medium">{project.year}</span>
          </div>
          <h3
            className="text-ink font-bold mb-1"
            style={{ fontSize: 'clamp(28px, 8vw, 42px)', fontFamily: "'Space Grotesk', sans-serif", letterSpacing: '-0.03em', lineHeight: 1 }}
          >
            {project.name}
          </h3>
          <p className="text-[11px] tracking-[0.2em] uppercase text-ink/40 font-medium mb-4">{project.type}</p>
          <p className="text-ink/60 leading-relaxed mb-4 text-[14px]">{project.description}</p>
          ...
          {project.href && (
            <span className="text-[11px] tracking-[0.2em] uppercase font-semibold text-ink underline underline-offset-4 decoration-ink/25">
              {project.status}
            </span>
          )}
        </div>
...
export default function Work() {
  return (
    <section id="work" className="bg-cream px-6 md:px-10 pt-24 md:pt-32 pb-24">
      <motion.div
